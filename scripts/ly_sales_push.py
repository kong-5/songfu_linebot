...
def build_customer_map(icpno: str, verbose: bool) -> dict:
    ...
    for r in rows:
        ...
        cmap[ct] = {
            ...
            "last_txn": str(r.get("CT_LAST_DT", "")).strip()[:10],   # 最後交易日（分辨舊客戶）
            "route": parse_route_from_addr(r),                        # 送貨地址 [N] → 路線
        }
    if verbose:
        n_rt = sum(1 for v in cmap.values() if v.get("route"))
        print(f"  客戶主檔對照：{len(cmap)} 筆（地址解析到路線 {n_rt} 筆）", flush=True)
    return cmap


_ROUTE_RE = re.compile(r"\[\s*([0-9A-Za-z]{1,6})\s*\]")


def parse_route_from_addr(r: dict) -> str:
    """從凌越送貨地址的 [N] 解析路線；優先 CT_ADDR3，再 ADDR1/ADDR2。取第一個中括號內容。"""
    for fld in ("CT_ADDR3", "CT_ADDR1", "CT_ADDR2"):
        m = _ROUTE_RE.search(str(r.get(fld, "")))
        if m:
...
def build_payload(icpno: str, date: str, verbose: bool) -> dict:
    ...
    if cmap:
        customers = [{"ct_no": ct, "name": info.get("name", ""), "fkfs": info.get("fkfs", ""),
                      ...
                      "route": info.get("route", ""), "last_txn": info.get("last_txn", "")}
