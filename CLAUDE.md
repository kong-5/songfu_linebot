...
- `docs/內網那台-Claude-Code-上手與同步.md`：內網 Windows 上開 Claude Code 時的上手包
  （三個資料夾別搞混、clone repo、雲端↔內網 分支同步流程、改內網腳本的正確順序）。
- `docs/凌越-CJSUM-庫存四捨五入.md`：**銷貨單金額對但庫存扣錯**（明細 `SD_QTY` 正確、
  實際扣庫存的 `CJSUM` 被四捨五入，凌越端問題不是我們送錯）；驗收腳本
  `scripts/ly_check_cjsum.py`（唯讀、跑在內網那台）＋要追問廠商的四件事。
