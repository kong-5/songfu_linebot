...
## 開發流程（agentic coding，2026-07-31 導入）
**動到寫入邏輯／DB／凌越串接／部署設定，一律照七步流程走，每步過閘門才進下一步**：
1 釐清需求 → 2 探勘定位 → 3 擬定計畫 → 4 實作 → 5 測試 → 6 自我審查 → 7 交付。
- 閘門的**唯一權威定義**在 `.claude/agentic-gates.json`（要調整就改那份，別另立一套）。
- 跑法：`/ac <任務>` 走完整流程、`/ac-verify <階段>` 單跑某步；終端機用 `npm run verify -- impl`。
- 步驟 1／2／3／6／7 是 **human 閘門＝要使用者回覆確認才往下**；4／5 自動檢查全綠即可自行往下。
- 使用者沒指定某步的驗證標準時，**先提出建議標準問過**，不要默默替他決定。
- 每次跑完把驗證結果留檔到 `docs/agentic-runs/`（`--log <檔案>` 會自動附加自動檢查結果）。
- hooks（`.claude/settings.json`）只警告不擋：改完 JS 自動 `node --check`、回合結束補跑步驟 4 閘門。
- 細節見 `docs/agentic-coding-流程.md`。改錯字／調文案這類小事不必七步跑完。
...
## 相關文件索引（細節在這裡）
- `docs/agentic-coding-流程.md`：七步開發流程、各步驗證閘門、怎麼改閘門、hooks 行為、留檔規則。
