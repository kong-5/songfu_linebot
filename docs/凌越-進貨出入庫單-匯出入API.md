...
## 14. 資料種類代碼掃描（2026-07-31）

### 14.1 ⚠ 判讀鐵則：`LyDataOut` 回 `rc=0` **不代表該資料種類存在**

實測陰性對照：拿 **`ZZZZZZ`／`0000ZZ` 這種絕不存在的代碼**去打 `LyDataOut`，
**同樣回 `rc=0`、總筆數 0、無欄位**。

> **因此判定資料種類是否存在，唯一可信的證據是「總筆數 > 0」或「回得出欄位」。**
> 空結果一律視為無法判定。`ly_newdoc_test.py discover` 已內建此陰性對照，
> 每次掃描前自動先驗一次並在對照失敗時明示警告。

（初版 discover 沒有對照組，把 `0000AN`／`0000AM` 的空結果誤判為「文件未列的新代碼」。）

### 14.2 掃描到的文件未列代碼

| 代碼 | 總筆數 | 欄位前綴 | 判讀 |
|---|---|---|---|
| `0000A3` | 60,867 | `I_*` | **確認存在**。`I_BANKNAME`/`I_BILLKIND`/`I_BKNO`/`I_ACALNO`/`I_ACSPNO`/`I_CASH_FG` → 票據／收付款類，**非採購單** |
| `0000A4` | 66 | `I_*` | **確認存在**，schema 與 A3 完全相同（推測為 A3 的對向單，如應收↔應付） |
| `0000AN`／`0000AM` | 0 | 無 | **無法判定**（rc=0 無鑑別力，很可能不存在） |

### 14.3 採購單：尚未找到，需凌越確認

矛盾點：進貨單的 `SP_ORDNO`（採購單號）實測 **69% 有值**、明細 `SD_ORDNO`+`SD_SEQFLD` **66% 有值**
→ 凌越系統內確實在用採購單，但**匯出入元件是否開放採購單的資料種類，目前無法從外部確認**。

**待問凌越（連同 §12.3 的 B6 匯入問題一起）**：

1. **採購單**的匯出入資料種類代碼是什麼？元件有無開放其 `LyDataIn`／`LyDataOut`？
   若有，可否提供欄位定義文件（格式同已提供的 0000AO）？
2. 元件對**未知的資料種類代碼**是否一律回 `rc=0`？（影響我方能否自行探測）
3. `0000A3`／`0000A4` 分別是什麼單別？

**在凌越回覆前**，若要繼續自行探測，唯一有效的方式是
`discover` 全掃並**只採信總筆數 > 0 的結果**——採購單若存在且在用，筆數必然可觀。
