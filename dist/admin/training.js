...
function registerTrainingRoutes(router, ctx) {
    ...
    function page(res, title, activeKey, subKey, icp, inner) {
        const body = `
        <style>
          /* [fix 2026-07-17] 刪除從 GET 連結改成 POST 表單（GET 直接刪資料可被跨站誘點觸發，
             SameSite=Lax 擋不住頂層導航）；此按鈕維持原連結外觀 */
          .tr-delform{display:inline;margin:0;}
          .tr-delbtn{background:none;border:none;padding:0;color:var(--accent,#2383e2);text-decoration:underline;cursor:pointer;font:inherit;}
        </style>
        ...
        const body = rows.map((r) => `
          <tr style="${r.status === "left" ? "opacity:.55;" : ""}">
            ...
            <td style="white-space:nowrap;"><a href="/admin/training/employees?c=${icp}&edit=${encodeURIComponent(r.id)}#empForm">編輯</a>
              | <form class="tr-delform" method="post" action="/admin/training/employees/${encodeURIComponent(r.id)}/delete?c=${icp}" onsubmit="return confirm('確定刪除此員工？其歷史簽到紀錄仍保留。')"><button type="submit" class="tr-delbtn">刪除</button></form></td>
          </tr>`).join("");
        const inner = `
        ${msgBar(req)}
        ${formCard}
    ...
    router.post("/training/employees/:id/delete", async (req, res) => {
        const icp = icpOf(req);
    ...
    router.get("/training/system", async (req, res) => {
        ...
        const checkRows = checks.map((c) => `
          <tr>
            ...
            <td><form class="tr-delform" method="post" action="/admin/training/system/pddro/${encodeURIComponent(c.id)}/delete?c=${icp}" onsubmit="return confirm('刪除此查核記錄？')"><button type="submit" class="tr-delbtn">刪除</button></form></td>
          </tr>`).join("");
        const inner = `
        ${msgBar(req)}
    ...
    router.post("/training/system/pddro/:id/delete", async (req, res) => {
        const icp = icpOf(req);
        ...
        const rows = plans.map((p) => {
            ...
            return `<tr>
              ...
              <td style="white-space:nowrap;"><a href="/admin/training/plans/${encodeURIComponent(p.id)}?c=${icp}">明細</a>
                | <form class="tr-delform" method="post" action="/admin/training/plans/${encodeURIComponent(p.id)}/delete?c=${icp}" onsubmit="return confirm('刪除此年度計畫及其所有排定項目？')"><button type="submit" class="tr-delbtn">刪除</button></form></td>
            </tr>`;
        }).join("");
        const inner = `
        ${msgBar(req)}
    ...
    router.post("/training/plans/:id/delete", async (req, res) => {
        const icp = icpOf(req);
        const doDel = async (h) => {
        ...
        const itemRows = items.map((i) => `
          <tr>
            ...
            <td style="white-space:nowrap;">
              ...
              | <form class="tr-delform" method="post" action="/admin/training/plans/items/${encodeURIComponent(i.id)}/delete?c=${icp}&plan=${encodeURIComponent(plan.id)}" onsubmit="return confirm('刪除此項目？')"><button type="submit" class="tr-delbtn">刪除</button></form>
            </td>
          </tr>`).join("");
        const inner = `
        ${msgBar(req)}
    ...
    router.post("/training/plans/items/:id/delete", async (req, res) => {
        const icp = icpOf(req);
        ...
        const rows = courses.map((c) => `
          <tr>
            ...
            <td style="white-space:nowrap;"><a href="/admin/training/courses/${encodeURIComponent(c.id)}?c=${icp}">明細</a>
              | <form class="tr-delform" method="post" action="/admin/training/courses/${encodeURIComponent(c.id)}/delete?c=${icp}" onsubmit="return confirm('刪除此課程及其簽到／滿意度／成效？')"><button type="submit" class="tr-delbtn">刪除</button></form></td>
          </tr>`).join("");
        const inner = `
        ${msgBar(req)}
        <div class="notion-card" style="margin-bottom:16px;">
    ...
    router.post("/training/courses/:id/delete", async (req, res) => {
        const icp = icpOf(req);
    ...
    router.get("/training/courses/:id", async (req, res) => {
        ...
        const surveyRows = surveys.map((s) => `
          <tr>
            ...
            <td>${esc(s.comment || "")}</td>
            <td><form class="tr-delform" method="post" action="/admin/training/courses/survey/${encodeURIComponent(s.id)}/delete?c=${icp}&course=${encodeURIComponent(c.id)}" onsubmit="return confirm('刪除此份問卷？')"><button type="submit" class="tr-delbtn">刪除</button></form></td>
          </tr>`).join("");
    ...
    router.post("/training/courses/survey/:id/delete", async (req, res) => {
        const icp = icpOf(req);
