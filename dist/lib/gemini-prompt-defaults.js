...
【七、劃掉與塗銷＝客戶取消（極重要，文字與圖像皆適用）】
- 凡是品項或數量被**橫線貫穿、打 X、圈起來再劃掉、明顯塗銷**者，視為客戶**取消該項**，**不要**輸出該品項到 JSON。
- 若無法明確判斷是「劃掉」還是書寫筆觸（例如僅有少量交錯線條），仍輸出該列，但 confidence_score 設為 50 以下，並在 remark 註明「可能被劃掉，請人工確認」。
- **禁止**忽略此條把劃掉品項當正常叫貨輸出 — 多送或誤送會造成客戶端食安／成本問題。

【八、修改塗改取「後寫」（最終值）】
- 若同一個數量格內見到「先寫的數字被劃掉、旁邊／下方再寫新數字」，請取**新寫的**那個；舊數字可在 remark 註明「原寫 X 已修改」。
- 同一品項列若有先 A 後 B 兩種單位（如先寫「2斤」劃掉改「2K」），以後寫為準。`;
...
【圖像專用：跨列／跨格手寫對齊（與【四】複合單位的反面禁令）】
- 手寫的數量／單位必須與**該列品名格的縱向中線**對齊。若兩個相鄰列（上下相鄰）都各自有手寫內容、彼此距離很近，**寧可拆成兩筆獨立品項**，**不要**把兩列的手寫合併成「一筆品項＋複合單位」。
- 【四】複合單位規則**僅適用於**「同一列、同一品名」內出現的多種寫法（如「1箱+5包」），絕對**不可**用來合併「上一列洋菇 3盒」與「下一列香菇 1kg」為單筆。
- 若不確定某行手寫屬於哪一列，以「**較貼近品名格中央／離下方分隔線較遠**」者為準；仍不確定時請輸出該列但 confidence_score ≤ 60、remark 註明「位置對應不確定」。

【圖像專用：字跡不清的退路與淡色預印辨識】
- 手寫無法明確辨認時，**仍**輸出該列（避免漏單），但 confidence_score 設為 60 以下，並在 remark 註明「字跡待人工確認」。**禁止**為了輸出而臆測具體數字或品名。
- 表單預印字常為**淡藍／淡灰／淡紅**色，**手寫**通常為**深黑／深藍／紅筆**。判斷該列是否有實際叫貨，**只看深色筆跡**；預印的提示文字（含 "kg" "公斤" "斤" 等預設單位）不算手寫，不可獨自構成叫貨記號。`;
